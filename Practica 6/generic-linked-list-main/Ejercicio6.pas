...
procedure Lectura(var r:reg_empleado);
    begin
        write('Ingrese anio: '); r.anio:= random(76)+1950; writeln(r.anio);
...
procedure Informar(L:Lista_empleados);
    begin
        L.reset();
        while (not L.eol()) do begin
            writeln('Nombre: ', L.current().nombre);
            writeln('Apellido: ', L.current().apellido);
            L.next();
        end;
    end;
